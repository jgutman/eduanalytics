...
```{r}
#how many cumulative amd postbac SCIENCE gpa's are missing
...
#how many cumulative amd postbac TOTAL gpa's are missing

gpa %>% 
  filter(aca_status_desc == "Cumulative" | aca_status_desc == "Postbaccalaureate Undergraduate") %>% 
  use_series(total_gpa) %>% 
  is.na() %>% 
  sum()



#who are missing science GPA -- should they be excluded
gpa %>% 
  filter(aca_status_desc == "Cumulative" | aca_status_desc == "Postbaccalaureate Undergraduate") %>% 
  filter(is.na(bcpm_gpa))



gpa %>% 
  filter(study_id == "a4611e1445da78b4cb6e937d2cc529b3")
