...
gpa_post_bac <- gpa %>% select(-ao_gpa, -ao_hours, -appl_year) %>%  
...
#remove those less than 12 total_post_back_hours
gpa_post_bac <- gpa_post_bac %>% mutate(did_postbac = ifelse(total_hours_postbac > 12, 1,0))

gpa_post_bac <- gpa_post_bac %>% select(-aca_status_desc, -total_hours_postbac)
...
gpa_grad <- gpa %>% 
  select(study_id, aca_status_desc, total_hours) %>%  
  filter(aca_status_desc == "Graduate") %>% 
  mutate(graduate_school_attended = ifelse(total_hours > 12, 1, 0)) %>% 
  select(study_id, graduate_school_attended)
...
totat_gpa_trend <- gpa %>% 
  ...
  select(study_id, aca_status_desc, total_gpa) %>% 
...
bcmp_gpa_trend <- gpa %>% 
  filter(aca_status_desc == "Freshman" | aca_status_desc == "Sophomore" | aca_status_desc == "Junior" | aca_status_desc == "Senior") %>%
  select(study_id, aca_status_desc, bcpm_gpa) %>% 
  spread(aca_status_desc, bcpm_gpa) %>% 
  mutate(bcmp_gpa_soph_higher = as.numeric(Sophomore > Freshman),
         bcmp_gpa_juni_higher = as.numeric(Junior > Freshman),
         bcmp_gpa_seni_higher = as.numeric(Senior > Junior)) %>% 
  select(-Freshman, -Sophomore, -Junior, -Senior) 
...
gpa_ready <- gpa_cumulative %>% 
  left_join(.,gpa_post_bac, by = "study_id") %>% 
  left_join(.,gpa_grad, by = "study_id") %>% 
  left_join(.,totat_gpa_trend, by = "study_id") %>% 
  left_join(., bcmp_gpa_trend, by = "study_id")
