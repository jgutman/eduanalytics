...
admissions_path <- "/Volumes/admissions/data_from_edudw_pull_nivedha" 
...
table(applications$appl_year, useNA = "ifany")
#2006-2017

table(applications$application_status, useNA = "ifany")
#Create accepted from Matriculated and Withdrawn after accepted! 
```
...
table(applications$application_status, applications$accepted, useNA = "ifany")
...
applications_rep_info <- applications %>% 
  left_join(app_repeated_count, by = "aamc_id") %>% 
  arrange(aamc_id, desc(appl_year)) %>%
  filter(appl_year >= 2014, appl_year <= 2017)

# This includes people who show up once in applications_rep_info but have applied more than once
...
mcat_new2 <- mcat_new %>%
  mutate(date = lubridate::dmy(new_mcat_test_date))
  distinct(aamc_id, date, .keep_all = TRUE)
...
```{r, further_analysis_mcat_old_new, eval = FALSE}
# I have the latest old mcat for each person and the latest new mcat for each person
...
table(as.factor(mcat_score_dates$old_mcat_latest), 
      as.factor(mcat_score_dates$new_mcat_latest))
...
mmi_pull_2 <- raw_files %>% 
  extract_data("MMI_Scores")

mmi_pull_2 %>%
  group_by(app_year) %>%
  summarize_all(funs(mean(is.na(.)))) %>%
  rename(aamc_id = amcas_id) %>%
  mutate(rank = if_else(app_year == 2013, 6L - rank, rank))
...
mmi_only <- mmi_pull_2 %>% filter(app_year != 2013)
#write this for final dataset
```
...
table(committee$app_year, useNA = "always")
...
xtabs <- purrr::partial(table, useNA = "ifany")
with(committee, xtabs(activity_text, faculty_activity))
with(committee, xtabs(activity_text, committee_chair_score_text))
with(committee, xtabs(committee_chair_score_text, faculty_activity))

with(committee, xtabs(committee_chair_score_text, app_year))
with(committee, xtabs(activity_text, app_year))

#No committee data prior to 2013
committee_intermediate <- filter(committee, app_year != 2012)
```

```{r}
committee_intermediate %>%
  filter(faculty_activity == "Screener") %>%
  group_by(appl_person_id) %>%
  distinct(appl_person_id, faculty_kerberos_id, .keep_all = TRUE) %>%
  summarize(n = n()) %>%
  {table(.$n)}
```
...
#What's the gpa from their primary institution? 
...
race_final <- race %>% 
  distinct(aamc_id, race, .keep_all = TRUE) %>% 
  dplyr::select(-appl_year)
...
#mmi
write_csv(mmi_only, "/Volumes/admissions/final_datasets/mmi_scores.csv")
```
