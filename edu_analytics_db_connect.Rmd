...
applicants_all_years %>% 
  mutate(committee_date = if_else(is.na(committee_date), NA_character_, paste0(committee_date, "0:00"))) %>%
  #mutate_at(vars(ends_with("_date"), ends_with("_day")), lubridate::mdy_hm) %>% 
  use_series(committee_date) %>% unique()
```
