...
get_src <- . %>% src_sql("mysql", ., info = dbGetInfo(.))
edu_db <- get_src(edu_db_con)
...
put_tbl_to_memory <- function(tbl_src, tbl_name) {
  tbl(tbl_src, tbl_name) %>%
  collect() %>%
  mutate_if(is.character, convert_or_retain)
}

table_names %>%
  str_detect(regex("hashed\\Wraw\\W\\d{4}_all_applicants$", ignore_case = TRUE)) %>%
  which() %>%
  extract(table_names, .) %>%
  map(function(tbl_name) put_tbl_to_memory(edu_db, tbl_name)) %>%
...
```{r}
tbl_names <- names(applicants_data) %>%
  paste("deidentified", "raw", ., sep = "$") %>%
...
applicants_data %>% write_to_database()
```
