...
bin_tables <- lapply(tbl, function(df) get_binary_tables(df, appl_year))
bin_kables <- lapply(bin_tables, function(t) {
  if (class(t)=="list") {
    lapply(t, function(x) kable(x, digits = 3, type='html'))
  } else {
    t
  }
}
...
The following tables display the number of distinct categories for each categorical variable by year. NAs are excluded.  
