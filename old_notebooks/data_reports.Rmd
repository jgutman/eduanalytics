...
# Creates an html file containing descriptive statistics for a single table
...
tab <- "deidentified$model_data$aoa_admissions"
...
kable(get_var_types_single(dat), type='html')
...
bin_tables <- get_binary_tables(dat, appl_year)

if (class(bin_tables)=="list") {
  lapply(bin_tables, function(x) kable(x, digits = 3, type='html'))
} else {
  bin_tables
}
...
The following table displays the number of distinct categories for each 
categorical variable by year, excluding NAs. 
...
# if no date variables in table should return informative message
