...
edudw_deidentified %<>%
  ...
  #filter_first_app_only(min_year = 2006, max_year = 2017) %>% 
  map(drop_empty_cols)
```
...
split_screener_score <- . %>% 
  ...
  mutate(screener_a = as.integer(screener_a),
         screener_b = as.integer(screener_b),
         screener_total = screener_a + screener_b)

applicant_data %<>%
  split_screener_score() %>%
  select(-screener_sd, -committee_sd, -scr_dec)
```
