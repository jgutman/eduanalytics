...
old_mcat_features <- old_mcat %>% 
  select(study_id, appl_year, test_date, total_percentile, bs_percentile, ps_percentile) %>% mutate (mcat_type = "old") %>% 
  rename(bio_percentile = bs_percentile, phy_chem_percentile = ps_percentile)
...
new_mcat_features <- new_mcat %>% 
  ...
  rename( test_date = new_mcat_test_date, total_percentile = new_mcat_total_percentile, bio_percentile = new_mcat_bbfl_percentile, phy_chem_percentile = new_mcat_cpbs_percentile) %>% 
  mutate(mcat_type = "new")
