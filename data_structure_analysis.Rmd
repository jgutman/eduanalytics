...
# number of categories for categorical variables by application year
...
format_cat_tables <- function(dat, varname, prop=T, digits = 3)   {
  table_list <- keep10_categories(get_cat_tables(dat, appl_year))
  if (prop) {
...
get_basic_date_summaries_single <- function(dat, varname) {
  ...
  dat %>% 
    group_by(!!quo_group_by) %>% 
    summarise_if(., is.POSIXct, funs(min, max, median, twentyfive_quantile, seventyfive_quantile), na.rm = TRUE)
}
