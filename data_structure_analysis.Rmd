...
classes <- dat %>% map(class) %>% sapply(function(x) x[1]) %>% unname
...
get_tables_single(dat, appl_year)
...
get_cat_tables <- function(dat, varname) {
  dat %>% 
    ...
    # rewrite without apply? use map instead? 
    # how to specify varname in function call?
    apply(., 2, table, dat$appl_year, useNA = "always") %>% 
    lapply(., t)
}

# formats tables -- only keep 10 largest categories
format_cat_tables <- function(table_list, prop=TRUE, digits = 3) {

  table_list <- lapply(names(table_list), function(x) {

    if (ncol(table_list[[x]]) > 10) {
  ...
  # rewrite this?
  if (prop) {
    # takes proportions instead of raw N's and rounds to 3 decimals
    table_list %<>% lapply(function(x) {
...
cat_tabs <- get_cat_tables(dat)
cat_tabs2 <- format_cat_tables(cat_tabs)
