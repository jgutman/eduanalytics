...
pct_miss_table <- function(dat) {
  dat %>%
    group_by(appl_year) %>%
    summarize_all(funs(pct_missing = mean(is.na(.)))) %>%
    column_to_rownames("appl_year") %>%
    t() %>% multiply_by(100) %>% round(2)
}

kable(pct_miss_table(dat), type='html', caption="Percent missing in each variable by year")
