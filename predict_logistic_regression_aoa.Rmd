...
all_preprocessing_steps <- rec %>%
  ...
  step_meanimpute(all_numeric(), -all_outcomes(), -appl_year) %>%
  step_center(all_numeric(), -all_outcomes(), -appl_year) %>%
  step_scale(all_numeric(), -all_outcomes(), -appl_year) %>%
  step_other(race) %>%
  step_dummy(all_nominal(), -has_role("id_var"))
...
mean((lasso.pred-aoa_test$outcome)^2)
