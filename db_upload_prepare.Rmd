...
write_to_database <- function(df_list, con) {
  df_list %>%
    ...
    dbWriteTable(edu_db_con, tbl_name, df, overwrite = TRUE))
}
...
```{r}
identified_raw_edudw_path <- data_dirs[[7]]
...
```{r}
identified_raw_edudw_list %>% write_to_database(edu_db_con)
```
