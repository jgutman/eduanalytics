...
```{r}
identified_raw_edudw_list$`identified$raw$experiences_2006_2012` %<>%
  mutate(n_weeks = time_length(end_dt - start_dt, unit = "week"))

identified_raw_edudw_list$`identified$raw$experiences_2013_2017` %<>%
  mutate(avg_hrs_per_week = if_else(appl_year >= 2015, NA_integer_, avg_hrs_per_week),
         n_weeks = time_length(end_dt - start_dt, unit = "week"))
```
