...
```{r, include = FALSE}
outcomes_dump_path <- file.path(parent_path, "outcome_data", "outcomes_version_1.sql") %>%
  list(dump_path = .)
...
```{r}
query_drop_tbl <- "drop table if exists ${new_tbl_name};"
query_rename_tbl <- "rename table ${old_tbl_name} to ${new_tbl_name};"

new_tbl_names <- old_tbl_names %>% 
  add_tbl_prefix(id = "identified", status = "outcome")

map_lgl(new_tbl_names, function(new) 
  interpolate_and_execute(query_drop_tbl, edu_db_con, commit = TRUE, 
                          new_tbl_name = new))

map2_lgl(old_tbl_names, new_tbl_names, function(old, new) 
  interpolate_and_execute(query_rename_tbl, edu_db_con, commit = TRUE, 
                          old_tbl_name = old, new_tbl_name = new))
```

```{r}
query_rename_col <- "alter table ${tbl_name} change ${old_name} ${new_name} DOUBLE;"
tbl_names <- get_tbl_names_by_stage(edu_db_con, "identified", "outcome")
...
tbl_names %>%
  ...
  map_lgl(function(tbl_name) interpolate_and_execute(query_rename_col, edu_db_con,
          commit = TRUE, tbl_name = tbl_name,                               
           old_name = "AAMC_ID", new_name = "aamc_id"))
```
